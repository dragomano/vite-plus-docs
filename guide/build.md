...
::: info
`vp build` всегда запускает встроенную продакшен-сборку Vite. Если в вашем проекте также определён сценарий `build` в `package.json`, используйте `vp run build`, когда требуется запустить именно этот сценарий. См. раздел [Встроенные команды и сценарии](/guide/run#built-in-commands-vs-scripts).
:::
