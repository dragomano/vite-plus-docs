...
3. `pnpm-lock.yaml`
4. `yarn.lock` или `.yarnrc.yml`
5. `package-lock.json`
6. `bun.lock` или `bun.lockb`
7. `.pnpmfile.cjs` или `pnpmfile.cjs`
8. `bunfig.toml`
9. `yarn.config.cjs`

Если ни один из этих файлов не найден, `vp` по умолчанию использует `pnpm`. Vite+ автоматически загружает соответствующий менеджер пакетов и использует его для выполнения запущенной команды.

Явно заданное поле `packageManager` также влияет на соответствующие shim-команды менеджеров пакетов. Если проект содержит `packageManager: "npm@10.9.4"`, то команды `npm` и `npx` будут использовать npm 10.9.4. Аналогично работают и другие пары алиасов: `pnpm`/`pnpx`, `yarn`/`yarnpkg` и `bun`/`bunx`. Несоответствующие инструменты не перенаправляются: например, `npm` в проекте на `pnpm` всё равно будет разрешаться как npm.
...
- `vp install --ignore-scripts` — пропускает lifecycle-скрипты
