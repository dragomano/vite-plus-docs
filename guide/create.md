...
- `--approve-builds` подтверждает выполнение и запускает заблокированные скрипты сборки зависимостей без запроса подтверждения
...
Информация об одобрении сохраняется в формате, ожидаемом соответствующим менеджером пакетов: `allowBuilds` для pnpm, `trustedDependencies` для bun или `dependenciesMeta.<pkg>.built` для yarn (в манифесте корня рабочего пространства). Транзитивные скрипты сборки, которые вы не выбирали (например, `esbuild`, подтянутый через Vite), остаются с настройками по умолчанию менеджера пакетов и не выводятся пользователю. В npm скрипты сборки выполняются по умолчанию, поэтому там ничего дополнительно одобрять не требуется.
