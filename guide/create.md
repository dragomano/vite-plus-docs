...
- `--approve-builds` подтверждает выполнение и запускает заблокированные сценарии сборки зависимостей без запроса подтверждения
...
### Сценарии сборки зависимостей {#dependency-build-scripts}

В целях безопасности pnpm, bun и yarn (Berry) не выполняют сценарии сборки зависимостей (`install` / `postinstall`, например нативную сборку пакетов вроде `better-sqlite3`), пока вы явно не подтвердите их выполнение. Если шаблон напрямую добавляет такую зависимость, `vp create` сообщает об этом после установки, вместо того чтобы оставлять проект в частично собранном состоянии:

- Интерактивный режим: вам будет предложено выбрать, какие из этих зависимостей следует одобрить и собрать (по умолчанию ничего не выбрано).
- Неинтерактивный режим: выводится примечание со списком таких зависимостей и указанием использовать `vp pm approve-builds`.
- `--approve-builds`: автоматически одобряет и выполняет их сборку, благодаря чему неинтерактивные запуски (например, в CI) могут создавать полностью готовый к использованию проект.

Информация об одобрении сохраняется в формате, ожидаемом соответствующим менеджером пакетов: `allowBuilds` для pnpm, `trustedDependencies` для bun или `dependenciesMeta.<pkg>.built` для yarn (в манифесте корня рабочего пространства). Транзитивные сценарии сборки, которые вы не выбирали (например, `esbuild`, подтянутый через Vite), остаются с настройками по умолчанию менеджера пакетов и не выводятся пользователю. В npm сценарии сборки выполняются по умолчанию, поэтому там ничего дополнительно одобрять не требуется.
...
- `produce()` возвращает [файлы](https://www.create.bingo/build/concepts/creations#files), которые нужно создать, а также необязательные [сценарии](https://www.create.bingo/build/concepts/creations#scripts) для запуска после генерации и [подсказки](https://www.create.bingo/build/concepts/creations#suggestions), которые будут показаны пользователю
...
Селектор всегда добавляет в конец пункт **Встроенные шаблоны Vite+**, чтобы шаблоны `vite:monorepo` / `vite:application` / `vite:library` / `vite:generator` оставались доступными из него. При выборе этого пункта происходит переход к стандартному встроенному потоку создания. Для сценариев и CI явные спецификаторы (`vp create vite:library`) обходят настроенный по умолчанию шаблон.
