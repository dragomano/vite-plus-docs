...
- `vite:generator` создаёт новый генератор кода (только для монорепозиториев, см. раздел [Генераторы кода](#code-generators))
...
- Используйте локальные шаблоны монорепозитория, объявленные в [`create.templates`](#code-generators) (например, внутренний генератор компонентов или сервисов)
...
- `--no-agent` пропускает настройку инструкций для агента
...
- `--package-manager <name>` использует указанный менеджер пакетов (`pnpm`, `npm`, `yarn` или `bun`)
- `--no-interactive` запускает без запросов
- `--verbose` показывает подробный вывод процесса генерации проекта
- `--list` выводит список доступных встроенных и популярных шаблонов
...
В монорепозиториях часто возникает необходимость создавать собственные строительные блоки: UI-компоненты, сервисы или внутренние пакеты, соответствующие принятым в проекте соглашениям. Vite+ поддерживает это с помощью пакетов-генераторов на основе шаблонов [Bingo](https://www.create.bingo/).

### Создание генератора {#scaffold-a-generator}

Внутри монорепозитория Vite+ выполните:

```bash
vp create vite:generator
```

Для этого требуется рабочее пространство монорепозитория. Если его ещё нет, сначала создайте его с помощью `vp create vite:monorepo`.

Созданный пакет генератора содержит:

- `src/template.ts` — определяет шаблон с помощью `createTemplate` из `bingo`: схему параметров, построенную на [Zod](https://zod.dev/), и функцию `produce()`, которая возвращает файлы для генерации
- `bin/index.ts` — CLI-точку входа на основе `runTemplateCLI` из Bingo

Если в монорепозитории есть родительский каталог с именем `generators` или `tools`, новый пакет по умолчанию будет создан в нём.

### Регистрация {#registration}

Локальные генераторы объявляются в [`create.templates`](/config/create#create-templates) в файле `vite.config.ts` монорепозитория. Это основной источник истины: в списке выбора `vp create` отображаются только зарегистрированные шаблоны.

Команда `vp create vite:generator` регистрирует генератор автоматически, добавляя запись в `create.templates` корневого файла `vite.config.ts`:
...
Повторный запуск идемпотентен (дублирующиеся записи не создаются), а существующее значение `create.defaultTemplate` сохраняется. Вы также можете добавлять записи вручную, например чтобы зарегистрировать шаблон, созданный другим способом. Значение `template` — это имя пакета генератора в рабочем пространстве или относительный путь к нему через `./path`.

### Запуск генератора {#run-a-generator}

Внутри монорепозитория выполните `vp create` и выберите генератор из списка шаблонов либо передайте его имя из записи `name` напрямую:

```bash
# В интерактивном режиме зарегистрированные локальные шаблоны
# отображаются вместе со встроенными
vp create

# Запуск зарегистрированного шаблона по его имени
vp create component

# Передача параметров генератору после --
vp create component -- --name @your-org/button
```

Если генератор зависит от `bingo`, Vite+ автоматически добавляет флаг `--skip-requests`, чтобы отключить исходящие сетевые запросы Bingo (например, обращения к API GitHub).

После выполнения генератора созданный пакет проходит стандартную интеграцию в монорепозиторий: регистрацию в рабочем пространстве, установку зависимостей и форматирование.

### Настройка генератора {#customize-a-generator}

Отредактируйте `src/template.ts`, чтобы определить параметры и файлы, которые должен создавать генератор:
...
- `options` определяет параметры и вопросы генератора с помощью схем Zod
- `produce()` возвращает [файлы](https://www.create.bingo/build/concepts/creations#files), которые нужно создать, а также необязательные [скрипты](https://www.create.bingo/build/concepts/creations#scripts) для запуска после генерации и [подсказки](https://www.create.bingo/build/concepts/creations#suggestions), которые будут показаны пользователю

Полное описание API шаблонов доступно в [документации Bingo](https://www.create.bingo/).
