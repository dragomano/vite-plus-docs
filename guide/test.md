...
:::info
`vp test` всегда запускает встроенную команду Vitest. Если в вашем проекте также определён сценарий `test` в `package.json`, используйте `vp run test`, когда требуется запустить именно этот сценарий. См. раздел [Встроенные команды и сценарии](/guide/run#built-in-commands-vs-scripts).
:::
