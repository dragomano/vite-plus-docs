...
Поддержка создания исполняемых файлов встроена в Vite+, поэтому отдельно устанавливать `@tsdown/exe` не требуется.

Сборка исполняемых файлов использует поддержку [Single Executable Applications](https://nodejs.org/api/single-executable-applications.html) в Node.js и требует Node.js версии 25.7.0 или новее. Если команда `vp pack --exe` сообщает о неподдерживаемой версии, переключите активную среду выполнения с помощью `vp env use 26`.
...
## Объединение CSS {#css-bundling}

Команда `vp pack` может преобразовывать и объединять CSS (включая CSS-модули и оптимизации с помощью [Lightning CSS](https://lightningcss.dev/)) для ваших точек входа. Эта возможность встроена в Vite+, поэтому отдельно устанавливать `@tsdown/css` или `lightningcss` не требуется — всё работает «из коробки».
