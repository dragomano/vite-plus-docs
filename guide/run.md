...
`vp run` запускает сценарии из `package.json` и задачи, определённые в `vite.config.ts`. Он работает аналогично `pnpm run`, но с встроенной поддержкой кэширования, порядка выполнения зависимостей и выполнения с учётом структуры рабочего пространства.
...
Используйте `vp run` с существующими сценариями из `package.json`:
...
`vp run build` выполняет соответствующий сценарий сборки:
...
## Встроенные команды и сценарии {#built-in-commands-vs-scripts}

`vp dev` — это встроенная команда. `vp run dev` — это ваш сценарий `dev`. Встроенные команды нельзя переопределить, поэтому добавление сценария `dev` не изменяет поведение `vp dev`:

| Команда                    | Что выполняется                                                             |
| -------------------------- | --------------------------------------------------------------------------- |
| `vp dev`                   | Встроенный сервер разработки Vite                                           |
| `vp run dev` / `vpr dev`   | Сценарий `dev` из `package.json` или задача `dev` из `vite.config.ts`       |
| `vp test`                  | Встроенная команда Vitest                                                   |
| `vp run test` / `vpr test` | Сценарий `test` из `package.json` или задача `test` из `vite.config.ts`     |

Команды `build`, `preview`, `lint`, `fmt`, `check` и `pack` работают аналогичным образом.

Если в проекте определён соответствующий сценарий или задача, запускайте их с помощью `vp run <name>`. Например, если в `package.json` определён сценарий `"dev": "astro dev"`, команда `vp run dev` запустит Astro, а `vp dev` проигнорирует этот сценарий и запустит Vite.

## Кэширование {#caching}

Сценарии из `package.json` по умолчанию не кэшируются. Используйте `--cache`, чтобы включить кэширование:
...
Команды, объединённые через `&&`, разделяются на независимые подзадачи. Каждая подзадача кэшируется отдельно, если [кэширование включено](/guide/cache#when-is-caching-enabled). Это работает как для задач, определённых в `vite.config.ts`, так и для сценариев из `package.json`:
...
Флаги также работают внутри вложенных сценариев. Например, `vp run -r build` внутри сценария разворачивается в отдельные задачи сборки для каждого пакета.

::: info
Распространённый шаблон в монорепозиториях — корневой сценарий, который рекурсивно запускает задачу:
