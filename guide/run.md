...
Используйте [`dependsOn`](#depends-on), чтобы запускать задачи в правильном порядке. При выполнении `vp run deploy` с конфигурацией выше сначала будут выполнены `build` и `test`. Зависимости также могут ссылаться на задачи из других пакетов того же проекта с помощью нотации `package#task`:
