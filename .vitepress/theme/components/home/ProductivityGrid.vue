<script setup lang="ts">
import RiveAnimation from '@components/shared/RiveAnimation.vue';
import focusOnShippingAnimation from '@local-assets/animations/514_x_246_focus_on_shipping_v2.riv';
import stayFastAtScaleAnimation from '@local-assets/animations/561_x_273_stay_fast_at_scale.riv';
import productivitySecurityImage from '@local-assets/productivity-security.png';
import tileOxc from '@local-assets/tiles/oxc.png';
import tileVite from '@local-assets/tiles/vite.png';
import tileVitest from '@local-assets/tiles/vitest.png';

import { trustedStackById } from '../../data/trusted-stack-stats';

const viteStack = trustedStackById('vite');
const vitestStack = trustedStackById('vitest');
const oxcStack = trustedStackById('oxc');
</script>

<template>
  <section
    class="wrapper wrapper--ticks border-t grid lg:grid-cols-2 divide-x divide-y divide-stroke"
  >
    <div class="p-5 sm:p-10 flex flex-col gap-3">
      <h5 class="text-balance sm:text-pretty">Проверенный стек для стандартизации</h5>
      <p class="sm:max-w-[28rem] text-pretty mb-12 sm:mb-16">
        Vite+ построен на признанных отраслевых стандартах с открытым исходным кодом и поддерживается теми же экспертами,
        которые стоят за этими проектами.
      </p>
      <div class="flex flex-col gap-6 sm:pr-10 -mx-5 sm:mx-0 divide-y sm:divide-y-0 divide-stroke">
        <div class="flex flex-col gap-6 pt-6 pb-6 sm:pb-0 px-5 sm:px-0 sm:pt-0 first:pt-0">
          <img loading="lazy" :src="tileVite" alt="Vite" class="w-24 flex-shrink-0 sm:hidden" />
          <div class="flex flex-row sm:items-center gap-6">
            <img
              loading="lazy"
              :src="tileVite"
              alt="Vite"
              class="w-24 flex-shrink-0 hidden sm:block"
            />
            <div
              class="relative flex flex-col flex-1 sm:pl-6 before:content-none sm:before:content-[''] before:absolute before:left-0 before:top-1/2 before:-translate-y-1/2 before:h-full before:w-px before:bg-stroke"
            >
              <p class="text-primary font-medium text-base sm:text-xl">
                {{ viteStack.npmWeeklyDownloadsDisplay }}
              </p>
              <p class="leading-tight text-base">Загрузок в неделю на npm</p>
            </div>
            <div
              class="relative flex flex-col flex-shrink-0 pl-6 before:content-[''] before:absolute before:left-0 before:top-1/2 before:-translate-y-1/2 before:h-full before:w-px before:bg-stroke"
            >
              <p class="text-primary font-medium text-base sm:text-xl">
                {{ viteStack.githubStarsDisplay }}
              </p>
              <p class="leading-tight text-base">Звёзд на GitHub</p>
            </div>
          </div>
        </div>
        <div class="flex flex-col gap-6 pt-6 pb-6 sm:pb-0 px-5 sm:px-0 sm:pt-0 first:pt-0">
          <img loading="lazy" :src="tileVitest" alt="Vitest" class="w-24 flex-shrink-0 sm:hidden" />
          <div class="flex flex-row sm:items-center gap-6">
            <img
              loading="lazy"
              :src="tileVitest"
              alt="Vitest"
              class="w-24 flex-shrink-0 hidden sm:block"
            />
            <div
              class="relative flex flex-col flex-1 sm:pl-6 before:content-none sm:before:content-[''] before:absolute before:left-0 before:top-1/2 before:-translate-y-1/2 before:h-full before:w-px before:bg-stroke"
            >
              <p class="text-primary font-medium text-base sm:text-xl">
                {{ vitestStack.npmWeeklyDownloadsDisplay }}
              </p>
              <p class="leading-tight text-base">Загрузок в неделю на npm</p>
            </div>
            <div
              class="relative flex flex-col flex-shrink-0 pl-6 before:content-[''] before:absolute before:left-0 before:top-1/2 before:-translate-y-1/2 before:h-full before:w-px before:bg-stroke"
            >
              <p class="text-primary font-medium text-base sm:text-xl">
                {{ vitestStack.githubStarsDisplay }}
              </p>
              <p class="leading-tight text-base">Звёзд на GitHub</p>
            </div>
          </div>
        </div>
        <div class="flex flex-col gap-6 pt-6 pb-6 sm:pb-0 px-5 sm:px-0 sm:pt-0 first:pt-0">
          <img loading="lazy" :src="tileOxc" alt="Oxc" class="w-24 flex-shrink-0 sm:hidden" />
          <div class="flex flex-row sm:items-center gap-6">
            <img
              loading="lazy"
              :src="tileOxc"
              alt="Oxc"
              class="w-24 flex-shrink-0 hidden sm:block"
            />
            <div
              class="relative flex flex-col flex-1 sm:pl-6 before:content-none sm:before:content-[''] before:absolute before:left-0 before:top-1/2 before:-translate-y-1/2 before:h-full before:w-px before:bg-stroke"
            >
              <p class="text-primary font-medium text-base sm:text-xl">
                {{ oxcStack.npmWeeklyDownloadsDisplay }}
              </p>
              <p class="leading-tight text-base">Загрузок в неделю на npm</p>
            </div>
            <div
              class="relative flex flex-col flex-shrink-0 pl-6 before:content-[''] before:absolute before:left-0 before:top-1/2 before:-translate-y-1/2 before:h-full before:w-px before:bg-stroke"
            >
              <p class="text-primary font-medium text-base sm:text-xl">
                {{ oxcStack.githubStarsDisplay }}
              </p>
              <p class="leading-tight text-base">Звёзд на GitHub</p>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div class="p-5 sm:p-10 flex flex-col gap-3 border-r-0">
      <h5>Сохраняйте скорость в масштабе</h5>
      <p class="max-w-[26rem] text-pretty">
        Благодаря низкоуровневым компонентам, написанным на Rust, Vite+ обеспечивает производительность корпоративного уровня:
        до <span class="text-primary">40× более быстрая сборка</span> по сравнению с webpack,
        <span class="text-primary">примерно от 50× до 100× более быстрый линтинг</span> по сравнению с ESLint
        и <span class="text-primary">до 30× более быстрое форматирование</span> по сравнению с Prettier.
      </p>
      <RiveAnimation
        :desktop-src="stayFastAtScaleAnimation"
        :desktop-width="561"
        :desktop-height="273"
        canvas-class="w-full mt-10"
      />
    </div>
    <div class="p-5 sm:p-10 flex flex-col gap-3">
      <h5>Сосредоточьтесь на релизах, а не на инструментах</h5>
      <ul class="list-disc pl-3 flex flex-col gap-2 marker:text-ruby mt-1">
        <li class="leading-tight">Перестаньте тратить время на поддержку инструментов</li>
        <li class="leading-tight">Упростите переход разработчиков между командами</li>
        <li class="leading-tight">
          Стандартизируйте лучшие практики для людей и рабочих процессов с помощью ИИ
        </li>
      </ul>
      <RiveAnimation
        :desktop-src="focusOnShippingAnimation"
        :desktop-width="514"
        :desktop-height="246"
        canvas-class="w-full mt-10"
      />
    </div>
    <div class="p-5 sm:p-10 flex flex-col gap-3">
      <h5>Безопасность цепочки поставок</h5>
      <p class="max-w-[20rem] text-pretty">
        Разработка Vite+ следует строгим практикам безопасности, и мы проверяем все его зависимости в рамках единой цепочки инструментов.
      </p>
      <img
        loading="lazy"
        class="w-full"
        :src="productivitySecurityImage"
        alt="Vite+ проверяет все зависимости в соответствии со строгими практиками безопасности"
      />
    </div>
  </section>
</template>

<style scoped></style>
