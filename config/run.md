...
Вы можете настроить Vite Task через раздел `run` в файле `vite.config.ts`. Подробнее о запуске сценариев и задач с помощью Vite+ см. в документации по [`vp run`](/guide/run).
...
Определяет, следует ли автоматически запускать сценарии `preX` и `postX` из `package.json` как хуки жизненного цикла при выполнении сценария `X`.

Если параметр включён (по умолчанию), запуск сценария, например `test`, автоматически выполнит `pretest` перед ним и `posttest` после него, если они определены в `package.json`.
...
export default defineConfig({
  run: {
    cache: {
      scripts: true, // Кэшировать сценарии из package.json (по умолчанию: false)
...
Значение `cache: true` включает кэширование как задач, так и сценариев, а `cache: false` отключает оба вида кэширования.
